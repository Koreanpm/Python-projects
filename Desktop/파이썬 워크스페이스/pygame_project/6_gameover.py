...
# 원본 공 이미지 로드 (가장 큰 공)
original_ball = pygame.image.load(os.path.join(image_path, "ball1.png"))  # 160x160

# 크기 조정하여 리스트에 저장
ball_images = [
    original_ball,  # 큰 공 (160x160)
    pygame.transform.scale(original_ball, (80, 80)),  # 중간 크기 (80x80)
    pygame.transform.scale(original_ball, (40, 40))   # 작은 크기 (40x40)
]
...
balls.append({
  ...
  "to_x": 1, #x축 이동방향, -3이면 왼쪽으로, 3이면 오른쪽으로
  "to_y": -6, #y축 이동방향
  ...
  for ball_idx, ball_val in enumerate(balls):
    ...
    for weapon_idx, weapon_val in enumerate(weapons):
      ...
      if weapon_rect.colliderect(ball_rect):
        ...
        if ball_img_idx < len(ball_images) - 1:
          #현재 공 크기 정보를 가지고 옴
          ...
          balls.append({
            ...
            ball_to_remove = ball_idx  # 현재 공을 제거하도록 설정
        break
      else: # 계속 게임을 진행
        continue # 안쪽 for 문 조건이 맞지 않으면 continue. 바깥 for 문 계속 수행
      break # 안쪽 for 문에서 break를 만나면 여기로 진입 가능. 2중 for 문
